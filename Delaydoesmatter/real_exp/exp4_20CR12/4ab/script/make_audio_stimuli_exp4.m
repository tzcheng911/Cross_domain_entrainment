...
close all
clc
cd('/Users/t.z.cheng/Google_Drive/Research/Delaydoesmatter/real_exp/exp4_20CR12/stimuli')
...
% cIOI = 0.35:step:0.4; % make examples
beginning = [-0.09, 0 , 0.09]; % Beginning modulation: early(-0.18s),ontime(0s),late(0.18s)
rampDur = 0.005;
delay = [1*sIOI+beginning;3*sIOI+beginning;5*sIOI+beginning]; % Delay duration: 2 or 4 or 8 IOI +/- 0.18
win_choice = 'linear'; 
...
%% Make just the comparison tones
tone_short = comparison{1};
tone_long = comparison{6};
filename = ['short_tone_1.wav'];
audiowrite(filename,tone_short,fs); 
filename = ['long_tone_6.wav'];
audiowrite(filename,tone_long,fs); 
...
%%
        for k = 1:length(begin_time)
            for l = 1:length(comp_IOI)
                ...
                filename = ['single_' '300ms_' 'delay_' delay_dur{m} '_' begin_time{k} comp_IOI{l} '.wav'];
                sound_file = [context zeros(1,round(fs*delay(m,k))) comparison{l}];
...
[s1empt,fs] = audioread('/Users/t.z.cheng/Google_Drive/Research/Delaydoesmatter/real_exp/exp2/stimuli/delay_2_81.wav');
[s1fill,fs] = audioread('/Users/t.z.cheng/Google_Drive/Research/Delaydoesmatter/real_exp/exp3/stimuli/linear_0.005_300ms/300ms_delay_2_81.wav');
[s1singlefill,fs] = audioread('/Users/t.z.cheng/Google_Drive/Research/Delaydoesmatter/real_exp/exp4/stimuli/single_300ms_delay_2_83.wav');
