...
clear 
clc
addpath('/Users/t.z.cheng/Documents/GitHub/Cross_domain_entrainment/Delaydoesmatter/real_exp/exp3_20CR11/script')
...
for i = 1:length(allspeech)
    ...
    speech_r = abs(speech);
    speech_r_p = speech.*(speech>0);
    speech_r_n = speech.*(speech<0);
    ...
    % Half-wave rectified speech does it make sense?????
%     [env_up_p,~] = envelope(speech_r_p,200,'peak'); % only the upper envelope
%     [env_up_n,~] = envelope(abs(speech_r_n),200,'peak'); % only the upper envelope
%     env_up_p(env_up_p < 0) = 0; % force the envelope to be zero
%     env_up_n(env_up_n < 0) = 0; % force the envelope to be zero
%     mod_pure_tone = pure_tone(:).*env_up(:);   
