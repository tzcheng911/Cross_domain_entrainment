...
sIOI = 0.60; %Standard IOI
%cIOI = [0.55, 0.57, 0.59, 0.61, 0.63, 0.65];% Comparison IOI: short(0.55, 0.57, 0.59), long(0.61, 0.63, 0.65)
cIOI = [0.5, 0.54, 0.58, 0.62, 0.66, 0.7];
beginning = [-0.18, 0 , 0.18]; % Beginning modulation: early(-0.18s),ontime(0s),late(0.18s)
delay = [sIOI+beginning;3*sIOI+beginning]; % Delay duration: 2 or 4 IOI +/- 0.18
...
context = repmat(pulse,1,6);
standard = repmat(pulse,1,2);
...
delay_dur = {'2','4'}; 
        ...
        for k = 1:length(begin_time)
            for l = 1:length(comp_IOI)
                ...
                filename = ['delay_' delay_dur{m} '_' begin_time{k} comp_IOI{l} '.wav'];
                sound_file = [context zeros(1,round(fs*delay(1,2))) standard zeros(1,round(fs*delay(m,k))) comp_int{l}];
                audiowrite(filename,sound_file,fs);
                end
            end
        end
