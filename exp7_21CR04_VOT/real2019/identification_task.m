    ...
    for i=1:length(VOT)
        for sex = 1:2
            for m = 1:numTrials
                ...
                output_result(ntrial,4) = 0; % response
                ntrial = ntrial + 1;
            end
        end
    end 
...
DrawFormattedText(wPtr, 'F: /pa/       J:/ba/','center','center', [0 0 0],[],[],[],2);
...
for plot_VOT = 1:length(VOT)
    ind = find((result(:,1) == plot_VOT) & (result(:,4) == 100)); % percent of reporting /pa/
...
lb = find(VOT(:)<round(perceptual_boundary),1,'last');
hb = find(VOT(:)>round(perceptual_boundary),1,'first');
lb
hb
...
save(fname,'subDetails','result','timerecord','lb','hb','perceptual_boundary');
