...
comp_IOI = [1,6]; % correspond to 0.55(1), 0.57(2), 0.59(3), 0.61(4), 0.63(5), 0.65(6) cIOI
numTrials= 3; %number of trials per subcondition
...
% Keyboard version with key locked
    ...
    if keyIsDown == 1
        RT = GetSecs - keytime;
     if aaa == 9 
        result(ntrial,5)=200; %longer
        result(ntrial,6)=RT;
     elseif aaa == 13 
        result(ntrial,5)=100; %shorter
        result(ntrial,6)=RT;
     end  
    end
...
%             end
 ...
 % draw response reminder
if (result(ntrial,4) == 1 && result(ntrial,5) == 100) || (result(ntrial,4) == 6 && result(ntrial,5) == 200)
    feedback = 'Correct';
elseif result(ntrial,4) == 1 && result(ntrial,5) == 200
    feedback = 'No, this one is shorter';
elseif result(ntrial,4) == 6 && result(ntrial,5) == 100
    feedback = 'No, this one is longer';
end

Screen('FillRect',wPtr,gray);
Screen('TextSize', wPtr ,50);
Screen('DrawText',wPtr,feedback,mx/2.5,my/2, [0 0 0]);
t5=Screen('Flip', wPtr,t4+1-slack); 

 t6=Screen('Flip', wPtr,t5+ITI-slack,doclear);
...
save(fname,'subDetails','result','timerecord','accuracy');
accuracy
